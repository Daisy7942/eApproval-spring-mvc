...
- 배포 주소 : http://3.34.97.153:8080/eapproval **(2026-08-22 ~ 08-25 한시 운영)**
  체험용 서버입니다. 비밀번호는 없으며, 로그인할 사번은 [확인용 계정](#확인용-계정) 을 참고해 주세요.
...
| DB 스키마 · SQL | 직접 |
| 화면 (JSP · CSS · jQuery) | 설계는 직접, 구현은 AI 활용 |

설계 판단과 검토는 직접 했으며, 구현 과정에서 문서와 AI 를 참고했습니다.
...
| 결재선 자동 추천 | 휴가신청서는 `manager_id` 를 타고 올라가 상급자 2단계를 미리 채움 |
...
결재선은 이 조직도에서 골라 넣습니다.
부서 · 팀은 별도 테이블로 나누고, 상급자는 `employee.manager_id` 로 자기참조해
팀원 → 팀장 → 본부장 → 대표이사 계층을 한 테이블로 표현했습니다.
사원 1,806명 · 5개 본부 · 14개 팀으로, 모두 실습용 가상 데이터입니다.
...
<details>
<summary><b>방법 2 — war 파일로 배포</b> (STS 없이 톰캣만으로 실행할 때)</summary>

<br>

완성된 웹앱을 다른 서버에 그대로 넘길 때 사용합니다.
